...
def sft_lora_run(params: dict) -> None:
    task = params.get("task", "")
    if "lora_train" in task:
        run_steps = [1, 2, 3]
    elif "lora_test" in task:
        run_steps = [4]
    else:
        print(f"task:{task} should be 'lora_train'/'lora_test'")
        raise SystemExit(1)
    ...
    if 1 in run_steps:
        ...
        _write_json(
            mmap_dir / "dataset_manifest.json",
            {"train_data_path": train_data, "backend": "hf_jsonl_lora"},
        )
    ...
    if 2 in run_steps:
        ...
        cfg_path = work_path / "_pipeline_lora_params.json"
        _write_json(cfg_path, train_payload)
        code = run_python_module(str(REPO_ROOT), "training.run_lora", ["--params", str(cfg_path)])
        if code != 0:
