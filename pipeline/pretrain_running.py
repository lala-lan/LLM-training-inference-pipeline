...
def pretrain_run(params: dict) -> None:
    task = params.get("task", "")
    if "pretrain" == task:
        run_steps = [1, 2, 3]
    elif task == "pretrain_test":
        run_steps = [4]
    else:
        print(f"task:{task} should be 'pretrain'/'pretrain_test'")
        raise SystemExit(1)
    ...
    if 1 in run_steps:
        ...
        _write_json(
            mmap_dir / "pretrain_manifest.json",
            {"train_data_path": train_data, "backend": "hf_text"},
        )
    ...
    train_payload = {
        ...
        "save_steps": int(params.get("save_steps", 1000)),
    ...
    if 2 in run_steps:
        ...
        cfg_path = work_path / "_pipeline_pretrain_params.json"
        _write_json(cfg_path, train_payload)
        code = run_python_module(str(REPO_ROOT), "training.run_pretrain", ["--params", str(cfg_path)])
        if code != 0:
