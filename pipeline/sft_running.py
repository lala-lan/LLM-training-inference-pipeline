...
def sft_run(params: dict) -> None:
    ...
    if "train" in task:
        ...
        print(f"task:{task} should be 'train'/'test'")
        raise SystemExit(1)
    ...
    if 1 in run_steps:
        ...
        _write_json(
            ...
            {"train_data_path": train_data, "backend": "hf_jsonl"},
        )
    ...
    if 2 in run_steps:
        ...
        cfg_path = work_path / "_pipeline_sft_params.json"
        _write_json(cfg_path, train_payload)
        code = run_python_module(str(REPO_ROOT), "training.run_sft", ["--params", str(cfg_path)])
        if code != 0:
